...
from .services import (
    ...
    get_partners_internal, filter_existing_leads, search_cpf_viper, search_cnpj_viper
)
from .models import Lead, Search, UserProfile
from .credit_service import debit_credits, check_credits
from .stripe_service import create_checkout_session, handle_webhook_event, CREDIT_PACKAGES
import csv
import json
            ...
            if quantity <= 0:
                ...
                for place in filtered_places[:quantity]:
                    ...
                    if cnpj:
                        ...
                        # 3. Buscar Sócios (API Interna/Secreta)
                        partners_data = get_partners_internal(cnpj)
                        if partners_data:
                            company_data['viper_data']['socios_qsa'] = partners_data
...
def export_leads_csv(request, search_id=None):
    ...
    # Middleware garante autenticação, mas verificamos por segurança
    if not user_profile:
        return redirect('login')
    ...
    # Filtrar leads do usuário
    leads = Lead.objects.filter(user=user_profile).order_by('-created_at')
    
    # Se search_id fornecido, filtrar por pesquisa
    if search_id:
        try:
